  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await dispatch(
        submitJobDetailsStep({ sessionId: JobStep1.data?.sessionId, details: payload })
      ).unwrap()
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-4">
          {["Remote", "Hybrid", "On-site"].map((mode) => (
            <div
              key={mode}
              className={`flex flex-col items-center rounded-md border p-4 cursor-pointer hover:border-primary hover:bg-primary/5 ${workMode === mode ? "border-primary bg-primary/10" : ""
                }`}
