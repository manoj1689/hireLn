...
export default function JobDetailsStep({
  ...
  const validate = () => {
    ...
    if (keyResponsibilities.length === 0)
      newErrors.keyResponsibilities = "At least one responsibility is required";
    if (!workMode) newErrors.workMode = "Select a work mode";
    if (!requiredExperience.trim())
      newErrors.requiredExperience = "Experience is required";
    if (!teamSize.trim()) newErrors.teamSize = "Select a team size";
    if (!reportingStructure.trim())
      newErrors.reportingStructure = "Select reporting structure";
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  useEffect(() => {
    if (!keyResponsibilityInput.trim()) return;

    const timer = setTimeout(() => {
      setKeyResponsibilities((prev) => [
        ...prev,
        keyResponsibilityInput.trim(),
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await dispatch(
        submitJobDetailsStep({
          sessionId: JobStep1.data?.sessionId || sessionId,
          details: payload,
        })
      ).unwrap();
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input
            ...
            placeholder="Type responsibility...wait 2 sec autosave"
            value={keyResponsibilityInput}
            onChange={(e) => {
              setKeyResponsibilityInput(e.target.value);
              setErrors((prev) => ({ ...prev, keyResponsibilities: "" }));
            }}
          />
        </div>
        ...
        {keyResponsibilities.length > 0 && (
          <span className=" flex text-sm mt-2 space-y-1 gap-4">
            {keyResponsibilities.map((item, index) => (
              <span
                key={index}
                className="flex justify-between items-center  text-xs bg-violet-400 text-white px-3 py-2 rounded-full"
              >
                {item}
                <button
                  type="button"
                  ...
                  onClick={() => {
                    const updated = keyResponsibilities.filter(
                      (_, i) => i !== index
                    );
      ...
      <div className="flex flex-col lg:flex-row w-full gap-4">
        ...
        <div className="space-y-2 lg:w-1/2">
          ...
          {errors.reportingStructure && (
            <p className="text-red-500 text-sm">{errors.reportingStructure}</p>
          )}
        </div>
      </div>
