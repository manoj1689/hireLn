...
export default function JobDetailsStep({ onSuccess, sessionId }: JobDetailsStepProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await dispatch(submitJobDetailsStep({ sessionId: JobStep1.data?.sessionId, details: payload })).unwrap();
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2">
        <Label htmlFor="keyResponsibility">Key Responsibilities</Label>
        <Input
        ...
        />
        <Button type="button" variant="outline" size="sm" className="mt-2" onClick={addResponsibility}>
          + Add
        </Button>
        {errors.keyResponsibilities && <p className="text-red-500 text-sm">{errors.keyResponsibilities}</p>}
      ...
      <div className="space-y-2">
        ...
        {errors.requiredExperience && <p className="text-red-500 text-sm">{errors.requiredExperience}</p>}
      </div>

      {/* Team Size */}
      <div className="space-y-2">
        <Label htmlFor="teamSize">Team Size</Label>
        <Select value={teamSize} onValueChange={(val) => {
          setTeamSize(val);
      ...
      <div className="space-y-2">
        ...
        <Select value={reportingStructure} onValueChange={(val) => {
