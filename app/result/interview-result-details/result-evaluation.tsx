              ...
              evaluations.map((e, idx) => (
                <Card key={idx} className="bg-muted/50">
                  <CardContent className="p-4 space-y-2">
                    <p className="font-medium text-sm text-muted-foreground">Q: {e.questionText}</p>
            ...
            <CardContent className="p-4 space-y-2">
              <div className="flex justify-between items-start">
                ...
                >
                  {isOpen ? (
                    ...
                    <>
                      Show Answer <ChevronDown className="ml-1 h-4 w-4" />
                    </>
                  )}
                </Button>
              </div>
              ...
              {isOpen && (
                <div className="bg-muted p-3 rounded-md text-sm">
                  <p className="font-medium mb-1">Candidate's Answer:</p>
                  <p className="text-muted-foreground">{e.answerText}</p>
                </div>
              )}

              <div className="grid grid-cols-2 gap-4 text-sm mt-3">
                <div className={getColor(e.factualAccuracy)}><strong>Factual:</strong> {e.factualAccuracy}</div>
                <div className={getColor(e.completeness)}><strong>Completeness:</strong> {e.completeness}</div>
                <div className={getColor(e.relevance)}><strong>Relevance:</strong> {e.relevance}</div>
                <div className={getColor(e.coherence)}><strong>Coherence:</strong> {e.coherence}</div>
              </div>
            </CardContent>
          </Card>
        )
          ...
          <CardContent>
            {evaluations.map((e, i) => (
              <div key={i} className="space-y-2 border-b pb-4 mb-4">
                ...
                <p className="text-sm text-muted-foreground">A: {e.answerText}</p>
              </div>
            ))}
          </CardContent>
        </Card>
