      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="bg-gradient-to-r from-pink-400 to-sky-400 text-white rounded-2xl shadow-lg p-6 flex justify-between items-start">
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold">{jobData.title}</h1>
            ...
            <div className="text-right">
              <p className="text-xl lg:text-2xl font-bold flex items-center gap-2 justify-end">
                <FaMoneyBillWave /> ₹{jobData.salaryMin.toLocaleString()} - ₹{jobData.salaryMax.toLocaleString()}
              </p>
              ...
              <span className="bg-green-500 text-white text-xs px-3 py-1 mt-2 inline-block rounded-full">
                {jobData.employmentType === "FULL_TIME" ? "Full Time" : jobData.employmentType}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <p className="text-gray-700 mb-4 leading-relaxed">{jobData.description}</p>

            {/* Tabs */}
            <div className="border-b flex gap-6 mb-4">
              {["overview", "responsibilities", "requirements", "skills"].map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`pb-2 font-semibold capitalize transition-colors ${activeTab === tab
            ...
            {activeTab === "skills" && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-red-50 p-4 rounded-md border">
                  <h5 className="text-red-600 font-semibold mb-2">Technical Skills</h5>
            ...
            <div className="mt-6">
              ...
              <ul className="list-disc list-inside text-sm">
                {jobData.languages.map(
                  (lang: { language: string; proficiency: string }, idx: number) => (
                    <li key={idx}>
        ...
        <div className="w-full lg:w-80 flex-shrink-0 space-y-4">
          ...
          <div className="bg-gradient-to-r from-pink-400 to-sky-400 text-white rounded-2xl p-5 shadow-lg">
            ...
            <p>
              ...
              <span
                className={`font-semibold ${jobData.status === "ACTIVE" ? "text-green-200" : "text-red-200"
                  }`}
              >
                {jobData.status}
              </span>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg text-center">
            ...
            <p className="text-gray-600 mb-4 leading-relaxed">
              ...
              <span className="font-medium text-blue-600">{jobData.title}</span> role.
